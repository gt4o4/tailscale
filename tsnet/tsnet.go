...
// running a separate `tailscaled` daemon or requiring any system-level configuration.
//
// # Overview
//
// Normally, Tailscale runs as a background system service (`tailscaled`) that manages
// a virtual network interface for the whole machine. `tsnet` takes a different approach:
// it runs a fully self-contained Tailscale node inside your process using a userspace
// TCP/IP stack (gVisor). This means:
//
...
//
//   - The node's [identity](https://tailscale.com/docs/concepts/tailscale-identity) and
//     state are stored in a directory you control.
//
// The core type is `Server`, which represents one embedded Tailscale node. Calling
// `Listen` or `Dial` on a `Server` routes traffic exclusively over the tailnet. The
// standard library's `net.Listener` and `net.Conn` interfaces are returned, so any
// existing Go HTTP server, gRPC server, or other net-based code works without
// modification.
//
// # Usage
//
