	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				c, err := routecheck.NewClient(t.Logf, b, b)
				if err != nil {
...
var _ routecheck.LocalBackend = &stubBackend{}
...
func (b *stubBackend) Peers() []tailcfg.NodeView {
	return b.peers
}
...
func (b *stubBackend) WatchNotifications(ctx context.Context, mask ipn.NotifyWatchOpt, onWatchAdded func(), fn func(roNotify *ipn.Notify) (keepGoing bool)) {
	if mask&ipn.NotifyInitialNetMap == 0 {
		panic("called without ipn.NotifyInitialNetMap in mask")
	}

	// WatchNotifications is used to wait for a valid NetMap.
	nm := b.NetMap()
	if nm == nil {
		panic("no netmap to wait for")
	}
