...
type relayManager struct {
	...
	// c is the [Conn] that owns this relayManager. It's set once during
	// [Conn] construction, before any relayManager method is called. It is
	// used by runLoop() to publish [Conn.hasPeerRelayServers] without
	// taking [Conn.mu].
	c *Conn
...
// publishHasServersRunLoop updates [Conn.hasPeerRelayServers] to reflect
// whether any relay servers are currently known. It is a no-op when r.c is
// nil, which only happens in unit tests that exercise relayManager directly
// without constructing a Conn.
func (r *relayManager) publishHasServersRunLoop() {
	if r.c == nil {
		return
	}
	r.c.hasPeerRelayServers.Store(len(r.serversByNodeKey) > 0)
}
