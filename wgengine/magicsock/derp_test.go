...
func TestForceSetNearestDERP(t *testing.T) {
	...
	if err := eventbustest.Expect(tw, func(e NewHomeDERP) error {
		if e.Old != 0 || e.New != 7 {
			return fmt.Errorf("got NewHomeDERP{Old:%d, New:%d}, want {Old:0, New:7}", e.Old, e.New)
		}
		return nil
	}); err != nil {
		t.Errorf("expected NewHomeDERP event: %v", err)
	}
}
